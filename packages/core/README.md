...
| `projection_engine.dart` | `ProjectionEngine` | ADR-0004: el store persiste, el engine despacha (filtro por eventType, checkpoints, idempotencia). El mismo despacho sirve al append síncrono, a `catchUp()` (backfill de projectors nuevos al arrancar, ADR-0010) y a `rebuild()` — la mecánica de la prueba ácida. |
