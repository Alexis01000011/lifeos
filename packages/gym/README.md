...
- **Domain events** (privados del módulo): `gym.workout_started` v1 · `gym.set_logged` v1 (nombre denormalizado, `exerciseId` opcional por weak schema, `weightKg` opcional — null = sin carga externa, ADR-0013 —, reps, `restBeforeSeconds` opcional) · `gym.set_removed` v1 (ADR-0010) · `gym.set_corrected` v1 (ADR-0012, identidad por posición) · `gym.workout_completed` v1 · `gym.workout_discarded` v1 (compensatorio, con `wasCompleted`) · `gym.set_logged_late` v1 (compensatorio, misma forma que `set_logged`) · `gym.exercise_added` v1 (con `legacyNames` y `modality` opcional, default weighted — ADR-0013) · `gym.exercise_renamed` v1 · `gym.exercise_muscle_group_corrected` v1 · `gym.exercise_modality_corrected` v1.
- **Comandos**: `StartWorkout`, `LogSet`, `CompleteWorkout`, `DiscardWorkout`, `AddMissedSet`, `RemoveLastSet`, `CorrectSet`, `AddExercise` (load-or-create del singleton), `RenameExercise`, `CorrectExerciseMuscleGroup`, `CorrectExerciseModality` — handlers que solo orquestan; las invariantes viven en el agregado:
  ...
  - serie: ejercicio no vacío, reps ≥ 1, peso ≥ 0 si viene (null = sin carga externa; la coherencia peso↔modalidad es de la UI, ADR-0013), descanso ≥ 0; el `exerciseId` NO se valida contra el catálogo (invariante entre agregados: la UI selecciona, llega válido por construcción)
...
- **Tablas propias** (ADR-0008, SQL crudo sobre la database compuesta): `gym_workout_history` (una fila por workout: inicio, fin, series, volumen — desde ADR-0013, volumen = carga externa), `gym_sets` (una fila por serie con su semana ISO, ADR-0010 — el volumen semanal es GROUP BY en la consulta; `weight_kg` nullable desde ADR-0013, con migración por pragma: la tabla vieja se tira y el projector renacido `gym.workout_sets_v2` la reconstruye en el catch-up), `gym_exercises` (con `modality`) y `gym_exercise_names` (ADR-0011: catálogo + mapa nombre normalizado → ejercicio con que se resuelven las series viejas a texto libre). `gym_sets` aún no lleva `exercise_id`: se agrega cuando la primera estadística lo consuma (mecánica de rename + catch-up, ADR-0010).
...
- **Hito 2**: dropsets (la identidad de serie ya existe, ADR-0012), semántica del descanso explícita, estadísticas de progresión (primera consumidora del `exercise_id` en `gym_sets` y del mapa de nombres, ADR-0011; por modalidad: PR de peso vs PR de reps, y series/grupo/semana como métrica transversal, ADR-0013).
