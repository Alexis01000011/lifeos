...
- [ ] Registro cómodo de series/reps/peso durante el entreno (UX de gimnasio: rápido, una mano)
  ...
  - **Feedback al registrar una serie** ✅ (2026-06-11): vibración háptica + destello en la fila nueva de la mini-lista al persistir el comando — el flujo app↔gym se sigue de reojo. Si el dominio rechaza, los campos conservan sus valores para corregir.
- [x] Mejoras de UX en el catálogo y el picker de ejercicios (2026-06-11)
  ...
  - **Grupos musculares colapsables en pantalla Ejercicios**: cada sección de grupo (p. ej. "Pecho", "Piernas") tiene un header colapsable. Originalmente expandidos al abrir; el feedback de uso real (2026-06-11) lo invirtió: **colapsados por default** y se expande solo el grupo que se quiere ver.
- [x] Pulido visual con feedback del primer uso real (2026-06-11): token `text-muted` aclarado para el contraste de labels/helpers de formularios (DESIGN.md alpha.1) y fix del overflow de la fila de contadores en Entrenar a 393dp
- [x] Ejercicios a peso corporal (2026-06-11, ADR-0013): modalidad por ejercicio en el catálogo (`weighted`/`bodyweight`, weak schema con default), `weightKg` opcional en las series (null = solo cuerpo, valor = lastre añadido; nunca 0 fantasma), la UI valida la coherencia peso↔modalidad, y "volumen" pasa a significar carga externa. En pantalla: `× 12` / `+5 kg × 12`. La estadística de progresión pendiente debe tratar modalidades por separado (PR de peso vs PR de reps) y usar series/grupo/semana como métrica transversal. **Al instalar en el A71**: la tabla de series renace sola (migración por pragma + catch-up del projector `gym.workout_sets_v2`) — verificar tras instalar que el historial sigue completo.
...
- [ ] Estadísticas de progresión: PRs, volumen por grupo muscular, tendencia por ejercicio — por modalidad (ADR-0013): PR de peso para weighted, PR de reps para bodyweight; series/grupo/semana como métrica transversal
